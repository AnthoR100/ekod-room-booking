        ...
        http
                ...
                .exceptionHandling(ex -> ex
                        .authenticationEntryPoint((request, response, e) -> {
                            ...
                            mapper.writeValue(response.getWriter(), Map.of("message", "Authentification requise."));
                        })
                        .accessDeniedHandler((request, response, e) -> {
                            response.setStatus(403);
                            response.setContentType(MediaType.APPLICATION_JSON_VALUE);
                            mapper.writeValue(response.getWriter(), Map.of("message", "Accès refusé : rôle insuffisant."));
                        })
                )
    ...
    public SecurityFilterChain viewFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .exceptionHandling(ex -> ex
                        ...
                        .accessDeniedHandler((request, response, e) -> response.sendRedirect("/access-denied"))
                )
                .addFilterBefore(jwtAuthenticationFilter, UsernamePasswordAuthenticationFilter.class);
